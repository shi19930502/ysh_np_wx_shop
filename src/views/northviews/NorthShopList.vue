									...
									<li class="flex flex-y-center flex-x-between" v-for="d in productList">
										...
										<div class="caigou_icon flex flex-y-center" v-if="d.skuMethodType == 2">
											<template v-if="d.quantity>0 || d.quantity=== ''">
												...
												<yd-input @change.native="quantityInput(d)" v-model="d.quantity" max="6" type="number" :show-success-icon="false"
												 :show-clear-icon="false" class="quantity-input"></yd-input>
											</template>
				...
				<div v-for="d in alreadyChosenGoods" class="flex flex-x-between flex-y-center">
					<yd-checklist-item :val="d.id" class="flex-item-6">
						<div style="padding: 12px 0;" class="flex flex-x-between flex-y-center">
							...
							<template v-else>
								<span v-if="d.proMeasureName != d.skuMeasureName">{{d.productName+'['+d.skuWeight+d.proMeasureName+'/'+d.skuMeasureName+']'}}</span>
			...
			<div class="yd-tabbar" slot="bottom">
				<div class="tabbar flex">
					...
					<div>
						...
						<div v-show="startAmount>=0" v-else class="tabbar-btn tabbarbtn-active flex flex-y-center flex-x-center" @click="toOrderSettlement2">去结算</div>
		...
		<yd-tabbar slot="tabbar">
			<div class="yd-tabbar">
				<div class="tabbar flex">
					<div class="flex flex-y-center">
						<!-- @click="getShopCar" -->
						<div @click="getShopCar" ref="gouwuche" class="status-icon flex flex-y-center flex-x-center" :style="alreadyChosenGoods.length == 0 ? 'background:#C2C2C2' : 'background:#6FB138'">
							<img src="../../assets/imgs/icon_caigou.png" style="width: 0.5rem;height:0.5rem;">
					...
					<div>
						<div style="right:1.6rem;border-right: 1px solid;background: #fff;" class="tabbar-btn flex flex-y-center flex-x-center">
						...
						<div v-else class="tabbar-btn tabbarbtn-active flex flex-y-center flex-x-center" @click="toOrderSettlement">去结算</div>
		...
		methods: {
			...
			add(d, isBtn) {
				...
				if (isBtn) {
					...
					let btn = new Btn('new' + d.id, startPoint, targetPoint, 7, d.skuPicUrl, 'left', () => {
						this.createShippingcar(d)
					})
					btn.create()
				} else {
